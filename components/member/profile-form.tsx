...
export function ProfileForm({ fullName, phone, member }: ProfileFormProps) {
  ...
  const {
    ...
    defaultValues: {
      ...
      bio: member?.bio ?? "",
      aiMotivationEnabled: member?.ai_motivation_enabled ?? true,
    },
  });
  ...
  const onSubmit = (values: MemberProfileFormValues) => {
    ...
    formData.set("aiMotivationEnabled", values.aiMotivationEnabled ? "on" : "");
    startTransition(() => {
      formAction(formData);
    });
  };
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-5" noValidate>
      <div className="grid gap-5 sm:grid-cols-2">
        ...
        <div className="flex flex-row items-center gap-3 sm:col-span-2 mt-2 rounded-lg border border-ink-border bg-ink-soft p-4">
          <div className="flex h-5 items-center">
            <input
              id="aiMotivationEnabled"
              type="checkbox"
              className="h-4 w-4 rounded border-ink-border bg-ink-bg text-primary focus:ring-primary focus:ring-offset-ink-bg"
              {...register("aiMotivationEnabled")}
            />
          </div>
          <div className="flex flex-col gap-1">
            <Label htmlFor="aiMotivationEnabled" className="text-base cursor-pointer">
              Enable AI Daily Motivations
            </Label>
            <p className="text-sm text-muted">
              Get unique, personalized motivational quotes generated by AI each day.
            </p>
          </div>
        </div>
      </div>
